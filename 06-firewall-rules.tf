...
resource "openstack_fw_rule_v2" "allow_internal" {
    ...
    action              = "allow"
    protocol            = "any"
    source_ip_address   = openstack_networking_subnet_v2.Subnet_1.cidr
...
resource "openstack_fw_group_v2" "firewall_group" {
    ...
    ports                      = [openstack_networking_port_v2.myport.id]
    depends_on                 = [openstack_fw_policy_v2.policy_in, openstack_fw_policy_v2.policy_out, openstack_networking_port_v2.myport]
}