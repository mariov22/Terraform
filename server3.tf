...
#   key_pair    = var.keypair_name     
  ...
  network {
    port = openstack_networking_port_v2.server3-net1.id
  }

  network {
    port = openstack_networking_port_v2.server3-net2.id  
  }

  user_data =  file("${path.module}/server1_config.yaml")
}

resource "openstack_networking_port_v2" "server3-net1" {
  name           = "port-server3-net1"
...
resource "openstack_networking_port_v2" "server3-net2" {
  name           = "port-server3-net2"
